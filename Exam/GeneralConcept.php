...
//Также окажется заэкранированным любой символ, который может быть заэкранирован. Иначе символ \ является обычным символом.
...
//шестнадцатиричным числом. Максимальное - это не больше двух. Если из три, то считается два, если меньше - то сколько есть.
...
//подмасок, то это будет воспринято как ссылка на 40ю подмаску. Сороковую - в десятичной системе счисления. Если же подмасок
//меньше, то это будет воспринято как символ с восьмеричном кодом 40.
...
//Например, '/(?<!foo)bar/' не найдёт вхождения "bar", которым не предшествует "foo". Т.е. qwefoobar этот шаблон
//проигнорирует, а asacdbar под него подойдет.
//(?<=\d{3})(?<!999)foo совпадает с подстрокой "foo", которой предшествуют три цифры, отличные от "999". Следует понимать,
//что каждое из утверждений проверяется относительно одной и той же позиции в обрабатываемом тексте.
//Утверждения могут быть вложенными, причем в произвольных сочетаниях: (?<=(?<!foo)bar)baz соответствует подстроке "baz",
//которой предшествует "bar", перед которой, в свою очередь, нет 'foo'.
...
//====================================================================================
//-------------------------------------------------------------- Работа с файлами и папками. Генерация, удаление, запись, обновление, права.
//====================================================================================
